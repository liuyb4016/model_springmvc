/**  
* @Title: RelecfClassUtils.java
* @Package cn.com.robusoft.mdesk.common.utils
...
public class ReflectClassUtils {
	private static final Logger logger = Slf4jLogUtils.getLogger(ReflectClassUtils.class);
	/**
	 * 
	* @Title: getAnnotationVvalue
	* @Description: 
	* @param @param portal  不能带‘/’
	* @param @param classStr  like  'cn.com.robusoft.mdesk.admin.controller.ActivityBasicnumController'
	* @param @return
		...
		if(clazzHeader==null){
			...
			for(Method method:clazz.getMethods()){
				...
				for(Annotation a:method.getAnnotations()){
					 ...
					 if(astr!=null && astr.startsWith("@org.springframework.web.bind.annotation.RequestMapping")){
						 ...
						 while(matcher.find()){ 
							 ...
							 String postType = matcher.group(4);
							 if(postType==null ||"".equals(postType)){
								 postType="GET";
							 }
							 methodUrl=tempUrl+";"+postType;
						 }
					 }
					 if(astr!=null && astr.startsWith("@cn.com.robusoft.mdesk.common.utils.MethodDirections")){
						 String regex = ".*value=(.*)\\)";
						 Pattern pattern=Pattern.compile(regex);
						 Matcher matcher=pattern.matcher(astr); 
						 while(matcher.find()){ 
				...
				if(methodUrl!=null && !"".equals(methodUrl)){
					list.add(methodUrl+"σ"+methodDirections);
				}
			}
			return list;
		}
